...
resource "aws_security_group" "petclinic-mutual-sg" {
  ...
  ingress {
    protocol = "tcp"
    from_port = 10250
    to_port = 10250 # brden fazla control plane icin gecerli, kubelet API kullanir,
    self = true # kendisinden gelen talepleri kabul et
  }
...
resource "aws_iam_instance_profile" "petclinic-master-server-profile" {
  name = "petclinic-master-server-profile-3"
